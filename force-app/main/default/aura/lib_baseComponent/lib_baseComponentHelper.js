({
    showToast : function(toastType,message,tmpParams) {
        ...
        if(showToast){
            console.log(Object.assign({ mode:'dismisible' },tmpParams));
            showToast.setParams(Object.assign({mode:'dismisible'},tmpParams)).fire();
        }
    },
    ...
    handleRequest: function(cmp,method,params,success,temOpt){
        debugger;
        var action = cmp.get('c.'+method);
        var options = Object.assign({
            spinningAttr: 'isProcessing',
            showSpinner: true,
            hideSpinner: true,
            handleErrors: true,
        },temOpt);
        ...
        if(errors && Array.isArray(errors)){
            ...
            errors.forEach(function(error){
                ...
                if(errorMessages.length > 0){
                    h.warning(errorMessages.join(', ',{mode: 'sticky', title: 'fix errors '}));
                    errorMessages = [];
                }
                if(error.fieldErrors && Array.isArray(error.fieldErrors)){
                    error.fieldErrors.forEach(function(errorList){
                ...
                if(error.fieldError && Array.isArray(error.fieldError)){
                    Object.keys(error.fieldError).forEach(function(field){
                        error.fieldError[field].forEach(function(errorList){
                            errorMessages.push(errorList.message);
                        });
                    });
                    if(errorMessages.length > 0){
                ...
                for(var statusCode=0,totalCode= statusKeys.length; statusCode< totalCode; statusCode++){
                    try{
                        console.log(message +'--'+statusKeys[statusCode]);
                        if(message.indexOf(statusKeys[statusCode]) > -1){
                            ...
                            if(!statusCodes[statusKeys[statusCode]]){
                                ...
                                message = statusCodes[statusKeys[statuCode]];
                            }
                            break;
                        }
    ...
    isValid: function(cmp,id){
        var validFields = cmp.find(id || 'validate');
        var isValid;
        if(validFields){
            isValid = [].concat(validFields).reduce(function(validFields,inputCmp){
